...
from callbacks_factories import ChangeSurveyStepsCallbackFactory
from db.service.abc_services import ABCServices
from enums import (SURVEY_STEP_VARIABLE_FILEDS, ListChangeSurveyStepsActions,
                   RedisTmpFields)
from keyboards_generators import get_keyboard_for_change_survey_steps
...
class ChangeSurveyStep(BaseCommand):
    ...
    async def execute(self, message: Message, state: FSMContext, command: Optional[CommandObject] = None, survey_id: int = None, step_id: int = None, **kwargs):
        ...
        await self.aiogram_wrapper.answer_massage(message=message,
                                                  text=text)
        keyboard = get_keyboard_for_change_survey_steps(field=self.filed_order[0]["field_name"])
    ...
    async def _send_field_request(self, state_context: FSMContext, message: Message, field_id: int):
        ...
        keyboard = get_keyboard_for_change_survey_steps(field=self.filed_order[field_id]["field_name"])
        send_message = await self.aiogram_wrapper.answer_massage(message=message,
    ...
    async def _keep_current_value(self, callback: CallbackQuery, callback_data: ChangeSurveyStepsCallbackFactory, state: FSMContext):
    ...
    async def _set_step_type(self, callback: CallbackQuery, callback_data: ChangeSurveyStepsCallbackFactory, state: FSMContext):
